...
[![QA](https://img.shields.io/badge/QA-173%2F173_A%2B-brightgreen?style=for-the-badge&logo=nodedotjs&logoColor=white)](QA-REPORT.md)
[![GitHub Actions](https://img.shields.io/badge/CI/CD-GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)](.github/workflows/qa.yml)
...
| Testing | [Node.js QA suite](run_tests.js) | 173 tests | — |
...
The repository ships a fully offline Node.js QA suite that parses `index.html` directly — no browser, no server, no network calls.

### Results

| Module | Tests | Covers |
|--------|-------|--------|
| HTML Structure | 18 | Single `<body>`, CDNs, no `console.log`, no cross-`<script>`-block `const` duplicates, no recursive `safeRRect`, file size bounds |
| Section Coverage | 82 | All 34 sections in HTML nav, 39 `newPage()` calls in PDF, all 39 Excel sheet names, Slides deck coverage |
| Financial Values | 38 | Canonical corrected values present, stale values removed (Altman Z, DuPont, assets/equity, SOFR→Fed Funds, NOPAT formula) |
| Export Functions | 23 | `exportPDF`/`exportSlides`/`exportXLSX` functions, PDF helpers, color constants, all 4 gap-fill sheet data rows, bare-array color arg check |
| Live Data Architecture | 12 | 5-tier fallback, `propagateLivePrice`, CORS proxies, Alpha Vantage, `getTimestamp`, mobile nav |
| **Total** | **173** | **100% pass rate — Grade A+** |
...
node run_tests.js          # full suite → writes QA-REPORT.md
make qa                    # same via Makefile
make fast                  # skip live-data tests
...
Every push to `main` and every pull request runs the full suite across **Node.js 22 and 24** in parallel. On a passing push to `main`, `QA-REPORT.md` is automatically committed back to the repo. Both workflow files set `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` to silence deprecation warnings from action internals.
...
.github/workflows/deploy.yml
  └─ build                        ← injects AV_API_KEY, uploads artifact
```

### Regression Guards

The suite encodes every bug found in this codebase as a non-regression test:

| Guard | Bug it prevents |
|-------|-----------------|
| `no_cross_script_block_duplicates` | Duplicate `<script>` block redeclaring `const gridOpts`/`D`/`C` — killed all 53 charts |
| `no_duplicate_const_in_export_pdf` | Modeling Lab block reusing `levHeaders`/`levXs`/`levData` — triggered `SyntaxError` in `exportPDF` |
| `no_console_log` | Debug `console.log()` calls left in production code |
| `no_recursive_safe_rrect` | `safeRRect` calling itself → infinite loop during PDF render |
| `excel_sheet_total_39` | Wrong sheet count if a sheet is accidentally added or removed |
| `pdf_newpage_count` | Fewer than 35 pages generated by `exportPDF` |
| `no_stale_altman_1_93` | Altman Z-score reverting to stale 1.93 (corrected value: 0.80) |
| `no_stale_sofr_5_30` | SOFR 5.30% re-appearing as primary rate (replaced by Fed Funds 4.35%) |
| `pdf_no_bare_array_color_args` | `setTextColor(C_INK)` without spread — crashed `exportPDF` with jsPDF `f3` invalid argument error |
...
├── .github/
│   └── workflows/
│       ├── deploy.yml                  # AWS Amplify deployment pipeline
│       │                               # Triggers on push to main, injects API key, uploads artifact
│       └── qa.yml                      # QA pipeline — Node 18/20/22 matrix
│                                        # Runs 172 tests on every push + PR, auto-commits QA-REPORT.md
├── run_tests.js                        # QA orchestrator (node run_tests.js)
├── Makefile                            # make qa / test / fast / t-html / t-data / ...
├── package.json                        # npm test scripts (devDep: node-html-parser)
├── tests/
│   ├── helpers.js                      # Shared getHTML(), assert(), contains() utilities
│   └── unit/
│       ├── test_html_structure.js      # 18 tests: tags, CDNs, script-block integrity
│       ├── test_section_coverage.js    # 82 tests: 34 sections × HTML + PDF + Excel + Slides
│       ├── test_financial_values.js    # 38 tests: canonical values, stale value removal
│       ├── test_export_functions.js    # 22 tests: export functions, helpers, color constants
│       └── test_live_data.js           # 12 tests: 5-tier fallback, propagateLivePrice
├── QA-REPORT.md                        # Auto-generated test report (172/172, A+)
